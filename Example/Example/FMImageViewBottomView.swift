...
//  funmee-app
//
//  Created by Hoang Trong Anh on 8/2/18.
...
class FMImageViewBottomView: UIView {
    ...
    func commonInit() {
        Bundle.main.loadNibNamed("FMImageViewBottomView", owner: self, options: nil)
        let button = UIButton()
        ...
        self.subAreaBottomView.append((button: button, label: label))
        ...
        for value in subAreaBottomView {
            ...
            value.button.heightAnchor.constraint(equalToConstant: 40 / 2).isActive = true
            value.button.widthAnchor.constraint(equalToConstant: 40 / 2).isActive = true
